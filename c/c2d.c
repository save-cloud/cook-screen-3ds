...
static Tex3DS_SubTexture g_icon_dsi_tex = {32, 32, 0.0f, 1.0f, 1.0f, 0.0f};
...
C2D_Image *c2d_raw_load_dsi_icon_from_buffer(u16 *iconData) {
  int WIDTH_POW2 = 32;
  int HEIGHT_POW2 = 32;

  C3D_Tex *ret = malloc(sizeof(C3D_Tex));
  C3D_TexInit(ret, WIDTH_POW2, HEIGHT_POW2, GPU_RGB565);

  memcpy(ret->data, iconData, 0x400 * 2);

  C2D_Image *image = malloc(sizeof(C2D_Image));
  image->tex = ret;
  image->subtex = &g_icon_dsi_tex;
