...
pub struct RdomStyle {
    ...
    pub scale: f32,
    pub scale_placeholder: Option<f32>,
    pub deep_3d: f32,
    pub z_index: f32,
        ...
        if let Some(attributes) = node_view.attributes() {
            for attr in attributes {
                match attr.attribute.name.as_str() {
                    ...
                    "scale_placeholder" => {
                        new.scale_placeholder = match attr.value {
                            OwnedAttributeValue::Float(scale) => Some(*scale as f32),
                            OwnedAttributeValue::Int(scale) => Some(*scale as f32),
                            _ => None,
                        }
                    }
                    "deep_3d" => {
